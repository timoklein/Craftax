...
def enforce_state_dtypes(state):
    ...
    return state.replace(
        player_health=jnp.int32(state.player_health),
        ...
        inventory=state.inventory.replace(
            ...
            wood_pickaxe=jnp.int32(state.inventory.wood_pickaxe),
            stone_pickaxe=jnp.int32(state.inventory.stone_pickaxe),
            iron_pickaxe=jnp.int32(state.inventory.iron_pickaxe),
            wood_sword=jnp.int32(state.inventory.wood_sword),
            stone_sword=jnp.int32(state.inventory.stone_sword),
            iron_sword=jnp.int32(state.inventory.iron_sword),
        ),
    )
...
def do_action(rng, state, action, static_params):
    ...
    _, is_attacking_zombie_array = jax.lax.scan(
        is_attacking_zombie_at_index, None, jnp.arange(static_params.max_zombies, dtype=jnp.int32)
    )
    ...
    new_zombie_health = new_zombies.health.at[target_zombie_index].add(-get_player_attack_damage(state) * is_attacking_zombie)
    ...
    did_kill_zombie = jnp.logical_and(old_mask, jnp.logical_not(new_zombies.mask[target_zombie_index]))
    new_achievements = state.achievements.at[Achievement.DEFEAT_ZOMBIE.value].set(
        jnp.logical_or(state.achievements[Achievement.DEFEAT_ZOMBIE.value], did_kill_zombie)
    )
    ...
    _, is_attacking_cow_array = jax.lax.scan(
        is_attacking_cow_at_index, None, jnp.arange(static_params.max_cows, dtype=jnp.int32)
    )
    is_attacking_cow = is_attacking_cow_array.sum() > 0
    target_cow_index = jnp.argmax(is_attacking_cow_array)
    new_cows = state.cows

    new_cow_health = new_cows.health.at[target_cow_index].add(-get_player_attack_damage(state) * is_attacking_cow)
    ...
    did_kill_cow = jnp.logical_and(old_mask, jnp.logical_not(new_cows.mask[target_cow_index]))
    ...
    new_food = jnp.where(did_kill_cow, jnp.minimum(9, state.player_food + 6), state.player_food)
    new_hunger = jnp.where(did_kill_cow, jnp.float32(0.0), state.player_hunger)
    ...
    _, is_attacking_skeleton_array = jax.lax.scan(
        is_attacking_skeleton_at_index, None, jnp.arange(static_params.max_skeletons, dtype=jnp.int32)
    )
    ...
    did_kill_skeleton = jnp.logical_and(old_mask, jnp.logical_not(new_skeletons.mask[target_skeleton_index]))
    new_achievements = state.achievements.at[Achievement.DEFEAT_SKELETON.value].set(
        jnp.logical_or(state.achievements[Achievement.DEFEAT_SKELETON.value], did_kill_skeleton)
    )

    state = state.replace(skeletons=new_skeletons)
    state = state.replace(achievements=new_achievements)

    did_attack_mob = jnp.logical_or(jnp.logical_or(is_attacking_zombie, is_attacking_cow), is_attacking_skeleton)

    did_kill_mob = jnp.logical_or(jnp.logical_or(did_kill_zombie, did_kill_cow), did_kill_skeleton)
    state = state.replace(
    ...
    new_inventory = new_inventory.replace(stone=state.inventory.stone + 1 * is_mining_stone)
    new_achievements = new_achievements.at[Achievement.COLLECT_STONE.value].set(
        jnp.logical_or(new_achievements[Achievement.COLLECT_STONE.value], is_mining_stone)
    )
    ...
    new_inventory = new_inventory.replace(coal=state.inventory.coal + 1 * is_mining_coal)
    ...
    new_inventory = new_inventory.replace(iron=state.inventory.iron + 1 * is_mining_iron)
    ...
    new_inventory = new_inventory.replace(diamond=state.inventory.diamond + 1 * is_mining_diamond)
    new_achievements = new_achievements.at[Achievement.COLLECT_DIAMOND.value].set(
        jnp.logical_or(new_achievements[Achievement.COLLECT_DIAMOND.value], is_mining_diamond)
    )
    ...
    new_inventory = new_inventory.replace(sapling=state.inventory.sapling + 1 * is_mining_sapling)
    new_achievements = new_achievements.at[Achievement.COLLECT_SAPLING.value].set(
        jnp.logical_or(new_achievements[Achievement.COLLECT_SAPLING.value], is_mining_sapling)
    )

    # Water
    is_drinking_water = state.map[block_position[0], block_position[1]] == BlockType.WATER.value
    new_drink = jnp.where(is_drinking_water, jnp.minimum(9, state.player_drink + 1), state.player_drink)
    ...
    new_achievements = new_achievements.at[Achievement.COLLECT_DRINK.value].set(
        jnp.logical_or(new_achievements[Achievement.COLLECT_DRINK.value], is_drinking_water)
    )

    # Plant
    is_eating_plant = state.map[block_position[0], block_position[1]] == BlockType.RIPE_PLANT.value
    new_plant = jnp.where(
        is_eating_plant,
    ...
    new_food = jnp.where(is_eating_plant, jnp.minimum(9, state.player_food + 4), state.player_food)
    ...
    new_map = jnp.where(action_block_in_bounds, new_map, state.map)
...
def do_crafting(state, action):
    ...
    is_crafting_iron_pickaxe = jnp.logical_and(
        ...
        jnp.logical_and(can_craft_iron_pickaxe, jnp.logical_and(is_at_furnace, is_at_crafting_table)),
    )
    ...
    is_crafting_iron_sword = jnp.logical_and(
        ...
        jnp.logical_and(can_craft_iron_sword, jnp.logical_and(is_at_furnace, is_at_crafting_table)),
    )
...
def place_block(state, action, static_params):
    ...
    is_placing_crafting_table = jnp.logical_and(
        ...
        jnp.logical_and(jnp.logical_not(is_in_wall(state, placing_block_position)), has_wood),
    )
    ...
    new_map = state.map.at[placing_block_position[0], placing_block_position[1]].set(placed_crafting_table_block)
    ...
    is_placing_furnace = jnp.logical_and(
        furnace_key_down,
        jnp.logical_and(jnp.logical_not(is_in_wall(state, placing_block_position)), has_stone),
    )
...
def update_mobs(rng, state, params, static_params):
    ...
    def _move_zombie(rng_and_state, zombie_index):
        ...
        random_move_proposed_position = zombies.position[zombie_index] + random_move_direction

        # Move towards player
        player_move_direction = jnp.zeros((2,), dtype=jnp.int32)
        player_move_direction_abs = jnp.abs(state.player_position - zombies.position[zombie_index])
        ...
        player_move_proposed_position = zombies.position[zombie_index] + player_move_direction

        # Choose movement
        close_to_player = jnp.sum(jnp.abs(zombies.position[zombie_index] - state.player_position)) < 10
        rng, _rng = jax.random.split(rng)
        ...
        is_attacking_player = jnp.sum(jnp.abs(zombies.position[zombie_index] - state.player_position)) == 1
        is_attacking_player = jnp.logical_and(is_attacking_player, zombies.attack_cooldown[zombie_index] <= 0)
        is_attacking_player = jnp.logical_and(is_attacking_player, zombies.mask[zombie_index])

        proposed_position = jnp.where(is_attacking_player, zombies.position[zombie_index], proposed_position)

        zombie_damage = jnp.where(
            state.is_sleeping,
            7,
            2,
        )
        new_cooldown = jnp.where(is_attacking_player, 5, zombies.attack_cooldown[zombie_index] - 1)
        ...
        position = jnp.where(valid_move, proposed_position, zombies.position[zombie_index])

        should_not_despawn = (
            jnp.abs(zombies.position[zombie_index] - state.player_position).sum() < params.mob_despawn_distance
        )
        ...
        state = state.replace(
            zombies=state.zombies.replace(
                ...
                attack_cooldown=state.zombies.attack_cooldown.at[zombie_index].set(new_cooldown),
    ...
    rng, _rng = jax.random.split(rng)
    (rng, new_state), _ = jax.lax.scan(_move_zombie, (rng, state), jnp.arange(static_params.max_zombies, dtype=jnp.int32))
    ...
    def _move_cow(rng_and_state, cow_index):
        ...
        valid_move = is_position_in_bounds_not_in_wall_not_in_mob_not_in_lava(state, proposed_position)
        position = jnp.where(valid_move, proposed_position, cows.position[cow_index])

        should_not_despawn = jnp.abs(cows.position[cow_index] - state.player_position).sum() < params.mob_despawn_distance

        # Clear our old entry if we are alive
        new_mob_map = state.mob_map.at[state.cows.position[cow_index, 0], state.cows.position[cow_index, 1]].set(
            jnp.logical_and(
                state.mob_map[state.cows.position[cow_index, 0], state.cows.position[cow_index, 1]],
        ...
        state = state.replace(
            cows=state.cows.replace(
                ...
                mask=state.cows.mask.at[cow_index].set(jnp.logical_and(state.cows.mask[cow_index], should_not_despawn)),
            ),
            mob_map=new_mob_map,
        )
    ...
    rng, _rng = jax.random.split(rng)
    (rng, new_state), _ = jax.lax.scan(_move_cow, (rng, new_state), jnp.arange(static_params.max_cows, dtype=jnp.int32))
    ...
    def _move_skeleton(rng_and_state, skeleton_index):
        ...
        random_move_proposed_position = skeletons.position[skeleton_index] + random_move_direction

        # Move towards player
        player_move_direction = jnp.zeros((2,), dtype=jnp.int32)
        player_move_direction_abs = jnp.abs(state.player_position - skeletons.position[skeleton_index])
        player_move_direction_index_p = (player_move_direction_abs == player_move_direction_abs.max()).astype(
            jnp.float32
        ) / player_move_direction_abs.sum()
        rng, _rng = jax.random.split(rng)
        ...
        player_move_towards_proposed_position = skeletons.position[skeleton_index] + player_move_direction
        player_move_away_proposed_position = skeletons.position[skeleton_index] - player_move_direction

        # Choose movement
        distance_to_player = jnp.sum(jnp.abs(skeletons.position[skeleton_index] - state.player_position))
        ...
        is_attacking_player = jnp.logical_or(
            ...
            jnp.logical_and(
                too_close_to_player,
                jnp.logical_not(is_position_in_bounds_not_in_wall_not_in_mob_not_in_lava(state, proposed_position)),
            ),
        )

        is_attacking_player = jnp.logical_and(is_attacking_player, skeletons.attack_cooldown[skeleton_index] <= 0)
        is_attacking_player = jnp.logical_and(is_attacking_player, skeletons.mask[skeleton_index])
        ...
        new_arrow_position = jnp.where(
            is_spawning_arrow,
            new_arrow_position,
            state.arrows.position[new_arrow_index],
        )
        new_arrow_mask = jnp.where(is_spawning_arrow, True, state.arrows.mask[new_arrow_index])
        new_arrow_direction = jnp.where(
            is_spawning_arrow,
        ...
        state = state.replace(
            arrows=new_arrows,
            arrow_directions=state.arrow_directions.at[new_arrow_index].set(new_arrow_direction),
        )

        proposed_position = jnp.where(is_attacking_player, skeletons.position[skeleton_index], proposed_position)

        new_cooldown = jnp.where(is_attacking_player, 4, skeletons.attack_cooldown[skeleton_index] - 1)

        valid_move = is_position_in_bounds_not_in_wall_not_in_mob_not_in_lava(state, proposed_position)

        position = jnp.where(valid_move, proposed_position, skeletons.position[skeleton_index])

        should_not_despawn = (
            jnp.abs(skeletons.position[skeleton_index] - state.player_position).sum() < params.mob_despawn_distance
        )
        ...
        new_mask = jnp.logical_and(state.skeletons.mask[skeleton_index], should_not_despawn)
        # Enter new entry if we are alive and not despawning this timestep
        new_mob_map = new_mob_map.at[position[0], position[1]].set(
            jnp.logical_or(new_mob_map[position[0], position[1]], new_mask)
        )
        ...
        state = state.replace(
            skeletons=state.skeletons.replace(
                ...
                attack_cooldown=state.skeletons.attack_cooldown.at[skeleton_index].set(new_cooldown),
                mask=state.skeletons.mask.at[skeleton_index].set(
                    jnp.logical_and(state.skeletons.mask[skeleton_index], should_not_despawn)
                ),
            ),
            mob_map=new_mob_map,
        )
    ...
    (rng, new_state), _ = jax.lax.scan(
        _move_skeleton, (rng, new_state), jnp.arange(static_params.max_skeletons, dtype=jnp.int32)
    )
    ...
    def _move_arrow(rng_and_state, arrow_index):
        ...
        proposed_position = arrows.position[arrow_index] + state.arrow_directions[arrow_index]
        ...
        in_wall = jnp.logical_and(
            in_wall,
            jnp.logical_not(state.map[proposed_position[0], proposed_position[1]] == BlockType.WATER.value),
        ...
        hit_player = jnp.logical_and(proposed_position_in_player, arrows.mask[arrow_index])
    ...
    rng, _rng = jax.random.split(rng)
    (rng, new_state), _ = jax.lax.scan(_move_arrow, (rng, new_state), jnp.arange(static_params.max_arrows, dtype=jnp.int32))

    return new_state


def get_distance_map(position, static_params):
    dist_x = jnp.abs(jnp.arange(0, static_params.map_size[0], dtype=jnp.int32) - position[0])
    dist_x = jnp.expand_dims(dist_x, axis=1)
    dist_x = jnp.tile(dist_x, (1, static_params.map_size[1]))

    dist_y = jnp.abs(jnp.arange(0, static_params.map_size[1], dtype=jnp.int32) - position[1])
...
def update_player_intrinsics(state, action):
    ...
    # Hunger
    hunger_add = jnp.where(state.is_sleeping, jnp.float32(0.5), jnp.float32(1.0))
    ...
    # Thirst
    thirst_add = jnp.where(state.is_sleeping, jnp.float32(0.5), jnp.float32(1.0))
    ...
    new_energy = jnp.where(new_fatigue > 30, jnp.maximum(state.player_energy - 1, 0), state.player_energy)
    new_fatigue = jnp.where(new_fatigue > 30, jnp.float32(0.0), new_fatigue)

    new_energy = jnp.where(new_fatigue < -10, jnp.minimum(state.player_energy + 1, 9), new_energy)
    ...
    recover_not_all = jnp.where(state.is_sleeping, jnp.float32(-0.5), jnp.float32(-1.0))
...
def update_plants(state, static_params):
    ...
    new_map, _ = jax.lax.scan(_set_plant_block, state.map, jnp.arange(static_params.max_growing_plants, dtype=jnp.int32))
...
def spawn_mobs(state, rng, params, static_params):
    ...
    rng, _rng = jax.random.split(rng)
    can_spawn_cow = jnp.logical_and(can_spawn_cow, jax.random.uniform(_rng, dtype=jnp.float32) < params.spawn_cow_chance)
    ...
    cows_can_spawn_map = jnp.logical_and(cows_can_spawn_map, player_distance_map < params.mob_despawn_distance)
    cows_can_spawn_map = jnp.logical_and(cows_can_spawn_map, jnp.logical_not(state.mob_map))
    ...
    cow_position = jax.random.choice(
        ...
        shape=(1,),
        p=jnp.reshape(cows_can_spawn_map, -1).astype(jnp.float32) / jnp.sum(cows_can_spawn_map),
    )
    ...
    new_cow_position = jnp.where(
        can_spawn_cow,
        cow_position,
        state.cows.position[new_cow_index],
    )

    new_cow_health = jnp.where(
        can_spawn_cow,
        params.cow_health,
        state.cows.health[new_cow_index],
    )

    new_cow_mask = jnp.where(
        can_spawn_cow,
        True,
    ...
    state = state.replace(
        ...
        mob_map=state.mob_map.at[new_cow_position[0], new_cow_position[1]].set(
            jnp.logical_or(state.mob_map[new_cow_position[0], new_cow_position[1]], new_cow_mask)
        ),
    )
    ...
    rng, _rng = jax.random.split(rng)
    zombie_spawn_chance = params.spawn_zombie_base_chance + params.spawn_zombie_night_chance * jnp.square(
        jnp.float32(1) - state.light_level
    )
    can_spawn_zombie = jnp.logical_and(can_spawn_zombie, jax.random.uniform(_rng, dtype=jnp.float32) < zombie_spawn_chance)
    ...
    zombies_can_spawn_map = jnp.logical_and(zombies_can_spawn_map, player_distance_map > 9)
    zombies_can_spawn_map = jnp.logical_and(zombies_can_spawn_map, player_distance_map < params.mob_despawn_distance)
    zombies_can_spawn_map = jnp.logical_and(zombies_can_spawn_map, jnp.logical_not(state.mob_map))

    can_spawn_zombie = jnp.logical_and(can_spawn_zombie, zombies_can_spawn_map.sum() > 0)
    ...
    zombie_position = jax.random.choice(
        ...
        jnp.arange(static_params.map_size[0] * static_params.map_size[1], dtype=jnp.int32),
        shape=(1,),
        p=jnp.reshape(zombies_can_spawn_map, -1).astype(jnp.float32) / jnp.sum(zombies_can_spawn_map),
    )
    ...
    can_spawn_skeleton = jnp.logical_and(
        can_spawn_skeleton, jax.random.uniform(_rng, dtype=jnp.float32) < params.spawn_skeleton_chance
    )

    skeletons_can_spawn_map = state.map == BlockType.PATH.value
    skeletons_can_spawn_map = jnp.logical_and(skeletons_can_spawn_map, player_distance_map > 9)
    skeletons_can_spawn_map = jnp.logical_and(skeletons_can_spawn_map, player_distance_map < params.mob_despawn_distance)
    skeletons_can_spawn_map = jnp.logical_and(skeletons_can_spawn_map, jnp.logical_not(state.mob_map))

    can_spawn_skeleton = jnp.logical_and(can_spawn_skeleton, skeletons_can_spawn_map.sum() > 0)
    ...
    skeleton_position = jax.random.choice(
        ...
        jnp.arange(static_params.map_size[0] * static_params.map_size[1], dtype=jnp.int32),
        shape=(1,),
        p=jnp.reshape(skeletons_can_spawn_map, -1).astype(jnp.float32) / jnp.sum(skeletons_can_spawn_map),
    )
    ...
    skeletons = Mobs(
        position=state.skeletons.position.at[new_skeleton_index].set(new_skeleton_position),
    ...
    state = state.replace(
        skeletons=skeletons,
        mob_map=state.mob_map.at[new_skeleton_position[0], new_skeleton_position[1]].set(
            jnp.logical_or(
...
def craftax_step(rng, state, action, params, static_params):
    ...
    # Reward
    achievement_reward = state.achievements.astype(jnp.float32).sum() - init_achievements.astype(jnp.float32).sum()
